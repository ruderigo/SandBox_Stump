...
def _create_socket(listen_ip, listen_port):
    ...
    # Resolved through getaddrinfo, not a raw tuple: MicroPython's
    # usocket rejects a plain (host, port) tuple here with "TypeError:
    # object with buffer protocol required". Same defect that stopped
    # the captive-portal DNS server from ever binding. This interface is
    # disabled in the shipped config, so it isn't currently reached --
    # fixed so it doesn't fail the moment somebody enables it.
    sock.bind(socket.getaddrinfo(listen_ip, listen_port)[0][-1])
    sock.settimeout(0)
    return sock
